...
class RegisterViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //Create a new calculator each time the view will appear. This is to account for taxation updates.
        calculator = createCalculaltor()
        updateTotal()
    }
...
extension RegisterViewController {
    ...
    fileprivate func createCalculaltor() -> BillCalculator {
        return BillCalculator(fetchTaxes())
    }
    
    private func billViewModel() -> BillViewModel? {
        ...
        for item in viewModel.orderItems {
            var taxType = BillItem.TaxType.standard
            if item.isTaxExempt {
                taxType = .exempt
            } else if item.category.contains("Alcohol") {
                taxType = .alcohol
            }
            billItems.append(BillItem(identifier: item.identifier, price: item.price as NSDecimalNumber, taxType: taxType))
        }
        
        let bill = calculator.bill(billItems, discounts: fetchDiscounts())
        
        return BillViewModel(bill)
    }
    
    private func fetchTaxes() -> [Tax] {
        
        var calculatorTaxes = [Tax]()
        for tax in taxes.filter({$0.isEnabled}) {
            if tax.label.contains("Alcohol") {
                calculatorTaxes.append(Tax.alcohol(tax.amount))
                continue
            }
            calculatorTaxes.append(Tax.standard(tax.amount))
        }
        
        return calculatorTaxes
    }
    
    private func fetchDiscounts() -> [Discount] {

        var calculatorDiscounts = [Discount]()
        for discount in discounts.filter({$0.isEnabled}) {
            if discount.isFlatCurrency {
                calculatorDiscounts.append(Discount.flatCurrency(NSDecimalNumber(value: discount.amount)))
                continue
            }
            calculatorDiscounts.append(Discount.percentage(discount.amount))
        }
    ...
    fileprivate func updateTotal() {
        ...
        subtotalLabel.text  = viewModel.displayTotalBillPreDiscountPreTaxes
        discountsLabel.text = viewModel.displayDiscountsApplied
        taxLabel.text       = viewModel.displayTaxesApplied
        totalLabel.text     = viewModel.displayTotalBill
    }
}
