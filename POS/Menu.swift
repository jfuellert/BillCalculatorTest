...
typealias Item = (identifier: UUID, name: String, category: String, price: NSDecimalNumber, isTaxExempt: Bool)

func category(_ category: String) -> (String, NSDecimalNumber) -> Item {
    return { name, price in
        return (UUID(), name, category, price, false)
    }
}
...
let discount5Dollars = (label: "$5.00", amount: 5.00, isEnabled: false, isFlatCurrency: true)
let discount10Percent = (label: "10%", amount: 0.10, isEnabled: false, isFlatCurrency: false)
let discount20Percent = (label: "20%", amount: 0.20, isEnabled: false, isFlatCurrency: false)
